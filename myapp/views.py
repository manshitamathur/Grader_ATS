...
from . models import Document, Document_ATS,Domains
...
#                 instance= Document.objects.get(id=my_id)
#                 instance.delete()
#                 #return render(request,'Results.html',context)
#                 return render(request,'star-rating.html',context)
...
#         return render(request,'GraderHomepage.html')
...
def ATS_HomePage1(request):
    try:
        ...
        if request.method == 'POST':
            ...
            #stop.extend(["AT_USER","URL","rt","corona","coronavirus","covid","amp","new","th","along","icai","would","today","asks"])
            stop={'down', 'was', 'she', 'theirselves', 'between', 'yon', 'by', 'such', 'whatsoever', 'why',
             'nought', 'while', 'itself', 'your', 'he', 'but', 'nothing', 'been', 'enough', 
             'theirs', 'whereunto', 'anything', 'yours', 'whom', 'wherefrom', 's', 'what', 
             'others', 'can', 'hers', 'only', 'anybody', 'into', 'whose', 'above', 'whosoever', 
             'did', 'had', 'it', 'don', 'few', 'whereto', 'several','she', 'you', 'whoever', 'being', 
             'our', 'were', 'too', 'once', 'again', 'theirself', 'does', 'ye', 'whomsoever', 'all',
              'them', 'their', 'through', 'me', 'whereinto', 'and', 'idem', 'wheresoever', 'they',
               'him', 'have', 'over', 'for', 'there', 'below', 'wherewithal', 'thine', 'whichsoever',
                'very', 'these', 'during', 'themselves', 'whosesoever', 'am', 'on', 'thyself', 'some',
                 'when', 'those', 'be', 'about', 'another', 'mine', 'ours', 'its', 'in', 'no', 
                 'himself', 'yourself', 'wherever', 'an', 'somewhat', 'are', 'somebody', 'we', 
                 'everyone', 'same', 'just', 'her', 'whichever', 'ought', 'any', 'youryours', 
                 'that', 'then', 'everybody', 'wherewith', 'most', 'whether', 'is', 'someone',
                  'of', 'than', 'who', 'so', 'having', 'ourself', 'where', 'either', 'one', 'whomever', 
                  'each', 'my', 'something', 'yourselves', 'as', 'out', 'until', 'before', 'whence', 
                  'myself', 'aught', 'thy', 'because', 'neither', 'how', 't', 'off', 'this', 'nobody',
                   'should', 'doing', 'here', 'thee', 'whereby', 'whereof', 'wherein', 'yonder', 'now',
                    'whereon', 'under', 'which', 'us', 'not', 'none', 'whomso', 'to', 'anyone', 'more',
                     'if', 'a', 'naught', 'ourselves', 'I', 'other', 'will', 'themself', 'with', 
                     'suchlike', 'against', 'herself', 'from', 'whatever', 'own', 'everything', 'up',
                      'the', 'his', 'whoso', 'after', 'nor', 'many', 'further', 'thou', 'whatnot', 'both',
                       'i', 'do', 'done', 'at', 'has','comfortable','work','best','look', 'or','able','behind','upon','on', 'whosever','is','has','being','have','had'}
            ...
            instance= Document_ATS.objects.get(id=my_id1)
            instance.delete()
    ...
    #return render(request, 'ATS_Homepage.html')
    return render(request, 'ATS_Homepg.html')
    ...
    try:
        if request.method == 'POST':
            ...
            with pdfplumber.open(f) as pdf:
                ...
                text=text1.lower()
                print(text.split())
                my_instance=Domains.objects.get(id=index)
                # jsonDec = json.decoder.JSONDecoder()
                # myPythonList = jsonDec.decode(Domains.Skills_list)
                # print(myPythonList)
                skills = 0
                projects = 0
                scores = []
                skills_list=[]
                projects_list=[]
                my_domains = {'Skills': my_instance.Skills_list.split(" "),'Projects': my_instance.Projects_List.split(" ")}
                for area in my_domains.keys():
                    if area == 'Skills':
                        for word in my_domains[area]:
                            if word in text.split():
                                skills+=1   
                                print(word)        
                        scores.append(skills)
            ...
            form = ResumeForm()
            my_list = Domains.objects.all() 
    #return render(request, 'home1.html')
            return render(request,'GraderHomepage.html',{'my_list':my_list})
